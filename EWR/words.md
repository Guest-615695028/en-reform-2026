# A Proposal on English Writing Reform - Words

## Adjectives
English Wiktionary has given a list of [English adjectives with gender inflections](https://en.wiktionary.org/wiki/Appendix:English_adjectives_that_inflect_for_gender), all of which are direct borrowings from Lattin languages such as French, Spannish, Italian, and Classical Lattin. 

## Defrancogallization
This is proposed as a contra-action to the French Academy suppressing Anglicisms. Notice the -ique, -eur/se, and others that accent the final syllable
|French imports|Latinly|Anglishly
|-|-|-
|counter-|contra-|wither-
|-ique|-ic|-y
|-eur|-or|-er

Oddnesses of derivation triggered by French are to be eliminated.
|Yesterday|Today|Why
|-|-|-
|comparison|comparation|Latin _comparatio_ <br>(compare + -ation)
|liaison|ligation|Latin _ligation_ <br>(compare + -ation)
_**Peeple** cumm across GitHub every day, but **some persons** would find this repository interesting.

4. Which Amerrican or British words shall be used?
As there is a larger peeple (or *there are more persons*, as seen beloe) in the United States than the United Kingdom, the Internet prefer American English than British English, but we are to be ballanced between them.

|American|Commonwealth|Ours|Note
|-|-|-|-
|-or|-our|-or|CSS uses `color`, not `colour`
|-ize|-ise/ize|-ize|confer Greek _-ίζω_
|-l-|-ll-|-l-|After unstressed vowel (commonly _-ed/-ing/-er_)
|-er|-re|-er|Pronounced as /ɚ/ or /əɹ/; CSS uses `center`, not `centre`
|-lyze|-lyse|-lyse|confer Greek _λύσις_
|defense|defence|defense|confer Latin _defensa_; allso _offense_
|license|licence|license|(_noun_) confer Latin _licentia_
|license|license|licenze|(_verb_) /ˈlaɪsənz/
|an herb|a herb|a herb, an 'erb|[_See here_](./pronunciation.md)
|Do you have _sth._?|Have you _sth._?|(Both)|[_See allso_](./grammar.md)

|Word|UK meaning|US meaning|Our meaning|Note
|-|-|-|-|-
|billion|10¹²|10⁹|(abandoned)|Write "one thousand million" insted.
|trillion|10¹⁸|10¹²|(abandoned)|Write "one twice-million" insted.

## Conjugational differences
|Infinitive|American past, past participle|British past, past participle
|-|-|-
burn|burned, burned|burnt, burnt
bust|bust, bust|busted, busted
dive|dove, dived|dived, dived
dream|dreamed, dreamed|dreamt, dreamt
got|got, gotten|got, got
kneel|kneeled, kneeled|knelt, knelt
lean|leaned, leaned|leant, leant
leap|leaped, leaped|leapt, leapt
learn|learned, learned|learnt, learnt
plead|pled, pled|pleaded, pleaded
saw|sawed, sawed|sawed, sawn
smell|smelled, smelled|smelt, smelt
spell|spelled, spelled|spelt, spelt
spill|spilled, spilled|spilt, spilt
spoil|spoiled, spoiled|spoilt, spoilt
